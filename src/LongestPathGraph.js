...
const LongestPathGraph = ({ type }) => {
  ...
  const handleNext = () => {
    ...
    setCurrentVis(
      showLongestPath(
        state.selectedSentenceVisualisation,
        state.testResults[type][activeStep]
      )
    );
  };
  ...
  return (
    <React.Fragment>
        ...
        <Graph
          ...
          getNetwork={(network) => {
            ...
            network.on("afterDrawing", function (ctx) {
              ...
              for (let node of currentVis.nodes) {
                if (node.type === "node") {
                  let nodeId = node.id;
      ...
      <MobileStepper
        variant="dots"
        steps={state.testResults[type].length}
        position="static"
        ...
        nextButton={
          <Button
            size="small"
            onClick={handleNext}
            disabled={activeStep === state.testResults[type].length - 1}
          >
            Next
            {theme.direction === "rtl" ? (
              <KeyboardArrowLeft />
...
export default LongestPathGraph;
